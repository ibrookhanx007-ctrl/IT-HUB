...
// for framer-motion's `delay`/`staggerChildren`. Long enough that a
// grid of cards visibly reveals left-to-right/top-to-bottom rather
// than all arriving at once.
export const STAGGER_STEP_SECONDS = 0.12;
...
export const cardHoverClassName =
  "relative transition-transform duration-300 group-hover:-translate-y-1.5 before:pointer-events-none before:absolute before:inset-0 before:rounded-[inherit] before:opacity-0 before:shadow-[0_0_0_1px_var(--color-gold),0_16px_36px_-8px_rgba(201,169,97,0.5)] before:transition-opacity before:duration-300 group-hover:before:opacity-100";
